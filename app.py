...
def index():
    ...
    if not currency:
        ...
        currency = requests.get('https://free.currencyconverterapi.com/api/v6/currencies&apiKey=%s' % app.config['API_KEY'])
        currency = currency.json()['results']
        for c in currency:
            price = requests.get('https://free.currencyconverterapi.com/api/v6/convert?q=USD_%s&apiKey=%s' % (c, app.config['API_KEY']))
            price_yesterday = requests.get(
                'https://free.currencyconverterapi.com/api/v6/convert?q=USD_%s&compact=ultra&date=%s-%s-%s&apiKey=%s' % (c, yesterday.year, yesterday.month, yesterday.day, app.config['API_KEY']))
